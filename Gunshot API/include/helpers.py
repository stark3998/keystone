...
def get_mel_spectrogram(
    file_path, mfcc_max_padding=0, n_fft=2048, hop_length=512, n_mels=128
):
    try:
        # Load audio file
        ...
        if mfcc_max_padding > 0 & shape < mfcc_max_padding:
            ...
            normalized_mel = np.pad(
                normalized_mel, pad_width=((0, 0), (xLeft, xRight)), mode="constant"
            )
...
def get_mfcc(file_path, mfcc_max_padding=0, n_mfcc=40):
    try:
        ...
        if mfcc_max_padding > 0 & shape < mfcc_max_padding:
            ...
            xLeft = xDiff // 2
            xRight = xDiff - xLeft
            normalized_mfcc = np.pad(
                normalized_mfcc, pad_width=((0, 0), (xLeft, xRight)), mode="constant"
            )
...
def play_dataset_sample(dataset_row, audio_path):
    ...
    file_path = os.path.join(
        audio_path, dataset_row.iloc[0]["fold"], dataset_row.iloc[0]["file"]
    )
...
def model_evaluation_report(model, X_train, y_train, X_test, y_test, calc_normal=True):
    ...
    print(dash)
    print(
        "{:<10s}{:>14.4f}{:>14.4f}".format(
            "Training:", train_score[0], 100 * train_score[1]
        )
    )
    print(
        "{:<10s}{:>14.4f}{:>14.4f}".format("Test:", test_score[0], 100 * test_score[1])
    )
...
def plot_train_history(history, x_ticks_vertical=False):
    ...
    plt.plot(
        max_accuracy_x, max_accuracy, marker="o", markersize=3, color="#1f77b4", alpha=7
    )
    plt.plot(
        max_val_accuracy_x,
        max_val_accuracy,
