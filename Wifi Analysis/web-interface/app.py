...
def location_from_router():
    return jsonify(SSID="Beesafe Router 1", RSSI="-10dBm")
...
def retrive_device(id):
    ...
    if store.get(id, None) is not None:
        return jsonify(json.dumps(store[id]))
...
def update_device():
    ...
    if store.get(result["id"], None) is not None:
        try:
            ...
            return jsonify(error="Internal server error", detail=str(err))
...
def register_device_on_map():
    ...
    d = Device(result["id"])
    util.save_device_info(result["id"], d)
    return jsonify(status=f'device with id {result["id"]} has been successfully saved')
